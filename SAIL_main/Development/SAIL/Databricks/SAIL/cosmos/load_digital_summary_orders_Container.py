...
,max_activity_name AS 
        (
			...
			INNER JOIN {digital_summary_milestone_activity} MA ON MA.is_deleted = 0 and tmpActivity.UPSOrderNumber = MA.UPSOrderNumber
			...
			INNER JOIN {map_milestone_activity} MMA ON MA.ActivityName = MMA.ActivityName     ----->sprint 54
				AND MA.SourceSystemKey = MMA.SOURCE_SYSTEM_KEY  --and WA.Type ='Out'	      ----->sprint 54
			LEFT OUTER JOIN {wh_wip_mapping_activity} WA ON MA.ActivityName = WA.ActivityName ----->sprint 54 changed to left join
...
,O.ORDER_REF_9_VALUE referenceNumber9    --Spint 55 UPSGLD-15868
,O.ORDER_REF_10_VALUE referenceNumber10  --Spint 55 UPSGLD-15868
,O.ORDER_REF_11_VALUE referenceNumber11  --Spint 55 UPSGLD-15868
,O.ORDER_REF_12_VALUE referenceNumber12  --Spint 55 UPSGLD-15868
